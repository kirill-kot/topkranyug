...
;
jQuery(function(e){if(typeof woocommerce_attr_slider_params==="undefined"){return false}e("input#min_attr, input#max_attr").hide();e(".attr_slider, .attr_label").show();var a=e(".attr_slider_amount #min_attr").data("min"),c=e(".attr_slider_amount #max_attr").data("max");if(woocommerce_attr_slider_params.min_attr){var d=parseInt(woocommerce_attr_slider_params.min_attr,10)}else{var d=parseInt(e("input#min_attr").val(),10);if(!d){d=parseInt(a,10)}}if(woocommerce_attr_slider_params.max_attr){var b=parseInt(woocommerce_attr_slider_params.max_attr,10)}else{var b=parseInt(e("input#max_attr").val(),10);if(!b){b=parseInt(c,10)}}e(document.body).bind("attr_slider_create attr_slider_slide",function(h,g,f){e(".attr_slider_amount .attr_label .from").html(d);e(".attr_slider_amount .attr_label .to").html(b);e(document.body).trigger("attr_slider_updated",[g,f])});e(".attr_slider").slider({range:true,animate:true,min:a,max:c,values:[d,b],create:function(){e(".attr_slider_amount #min_attr").val(d);e(".attr_slider_amount #max_attr").val(b);e(document.body).trigger("attr_slider_create",[d,b])},slide:function(f,g){e(".attr_slider_amount .attr_label .from").html(g.values[0]);e(".attr_slider_amount .attr_label .to").html(g.values[1]);e("input#min_attr").val(g.values[0]);e("input#max_attr").val(g.values[1])},change:function(f,g){e(document.body).trigger("attr_slider_change",[g.values[0],g.values[1]])}})});
...
/*!
 * Generated using the Bootstrap Customizer (//getbootstrap.com/customize/?id=dfbbba60ac39f88c4a74)
 * Config saved to config.json and //gist.github.com/dfbbba60ac39f88c4a74
 */
...
var data,gradient,chart,i,j,$this=$(this),ctx=$this.find("canvas")[0].getContext("2d"),options={showTooltips:$this.data("vcTooltips"),animationEasing:$this.data("vcAnimation"),datasetFill:!0,scaleLabel:function(object){return" "+object.value},responsive:!0},color_keys=["fillColor","strokeColor","highlightFill","highlightFill","pointHighlightFill","pointHighlightStroke"];for($this.data("chart")&&($this.data("chart").destroy(),$this.removeData("animated")),data=$this.data("vcValues"),ctx.canvas.width=$this.data('width')?$this.data('width'):$this.width(),ctx.canvas.height=$this.data('height')?$this.data('height'):$this.width(),i=data.datasets.length-1;i>=0;i--)for(j=color_keys.length-1;j>=0;j--)"object"==typeof data.datasets[i][color_keys[j]]&&2===data.datasets[i][color_keys[j]].length&&(gradient=ctx.createLinearGradient(0,0,0,ctx.canvas.height),gradient.addColorStop(0,data.datasets[i][color_keys[j]][0]),gradient.addColorStop(1,data.datasets[i][color_keys[j]][1]),data.datasets[i][color_keys[j]]=gradient);waypoint?$this.waypoint($.proxy(addchart,$this),{offset:"85%"}):addchart()}),this};window.vc_line_charts=function(model_id){var selector=".vc_line-chart";"undefined"!=typeof model_id&&(selector='[data-model-id="'+model_id+'"] '+selector),$(selector).saturnLineChart()};$('.product-item .img-wrapper img').each(function(){var preHeight=$('.img-wrapper').height(),imageHeight=$(this).height(),preWidth=$('.img-wrapper').width(),imageWidth=$(this).width();$(this).attr({'width':'','height':''}).css({'width':$('.img-wrapper').width()})});$('.type-product img').each(function(){$(this).attr({'width':'','height':''})});var pathname=window.location.pathname;var url=window.location.href;if(pathname==='/careers/'){$('.career-title').each(function(){$('#workApply .work-list select').prepend('<option value="'+$(this).text()+'">'+$(this).text()+'</option>')});$('body').find('a.career-apply').attr('data-toggle',"modal").attr('data-target',"#workApply").text('Откликнуться')}
$('.onTheFront img').each(function(){$(this).attr({'width':'','height':'','sizes':''})});var currModal=$('body').find('.modal'),btn=$('body').find('.modal').find('.button-block').prop('disabled',!0),checkbox=currModal.find('[type="checkbox"]').prop('checked',!"checked"),persDataBtn=$('#persData').find('.button-primary').prop('disabled',!1);if(currModal.length){if(currModal.find('.g-recaptcha').length){$('.g-recaptcha').find('div').removeAttr("style")}
$('.modal .check label').find('br').remove();checkbox.on('click',function(){$('#persData .button-block').text('Принимаю');checkbox.each(function(){var $this=$(this);var $thisBtn=currModal.find('.button-block');$this.prop("checked",!$this.prop("checked"));if($this.prop("checked","checked")){$thisBtn.prop('disabled',!1)}else{$thisBtn.prop('disabled',!0)}})});persDataBtn.on('click',function(){currModal.find('[type="checkbox"]').prop("checked","checked");currModal.find('.button-block').prop('disabled',!1);currModal.css({'overflow-y':'auto'})})}
$('#footerPD').click(function(){$('#persData .button-block').text('Закрыть')})})(jQuery)
...
//github.com/imakewebthings/jquery-waypoints/blob/master/licenses.txt
*/
